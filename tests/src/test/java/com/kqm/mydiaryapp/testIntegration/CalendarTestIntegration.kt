...
class CalendarTestIntegration {
    ...
    @Test
    fun `when call add quote should add a quote`(): Unit = runTest {
        ...
        viewModel.calendarWithQuotes.asSnapshot().asFlow().test {
            assertEquals(expectedDay, awaitItem().months[0].days[2])
        }
    }
