...
private fun QuoteType.toConvertName(): String {
    return when (this) {
        ...
        QuoteType.FAMILIAR -> "Familiar"
        QuoteType.AMISTAD -> "Amigos"
        QuoteType.TRABAJO -> "Trabajo"
    }
}
...
fun LocalQuote.toQuote(): Quote {
    val quoteType = when(typeQuote) {
        "Personal" -> QuoteType.PERSONAL
        "Familiar" -> QuoteType.FAMILIAR
        "Amigos" -> QuoteType.AMISTAD
        "Trabajo" -> QuoteType.TRABAJO
        else -> QuoteType.PERSONAL
    }
    return Quote(
        id = id,
        hour = hour,
